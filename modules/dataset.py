...
class TorchDataset(Dataset):
    def __init__(
            self, data, col_type,
            vocab, embedding_dim, use_pos, mask_rate, replace_rate, n_sample, mode='test'):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            sample['input']['idx'] = idx
            sample['input']['weight'] = weight
        else:
            sample['gathering'] = {}
...
def create_dataloader(
        ...
        mask_rate=0, replace_rate=0, n_sample=0, shuffle=False, mode='test'):

    dataset = TorchDataset(
        data, col_type, vocab, embedding_dim, use_pos,
        mask_rate, replace_rate, n_sample, mode)
